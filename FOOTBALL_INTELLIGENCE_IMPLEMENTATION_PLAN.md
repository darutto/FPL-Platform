...
| FI-8 | S3 — discovery: competitions, seasons, fixtures | complete | sportmonks-client **128 passed** (107 pre-existing intact, +21 new) | Added `trial_entities.py` (sweeps all 15 `ENDPOINTS` families, reporting per family reachable/empty/unavailable, record count, and the `provider_id` set actually observed; objective 1 status derived from the `leagues`/`seasons` families specifically) and `trial_fixtures.py` (objective 2 Premier League season fixtures and objective 3 cross-competition fixtures for Premier League clubs, kept as two separately-observable facts — cross-competition status requires both a matching participant and a league id other than the Premier League's, not just a non-empty result). Both exit 1 when a family/query is swapped for `edge_cases.json`'s empty envelope, with the degradation and the affected shape entry isolated to the swapped fact and every other entry unchanged (proven per construction site by a same-run baseline comparison). `TRIAL_STATUS.md` objectives 1–3 point at these scripts with mock-only evidence pointers, mirroring objective 17's. No change to `_trial_common.py`, `trial_auth.py`, `client.py`, `transport.py`, `config.py`, `models.py`, or `conftest.py`. |
