...
11. **Every derived value is proven derived by the two-input equality test below.** Item 10 says *what* must be derived; this says what counts as proof. Applies to every `evidence` string and every `observed_shapes[]` entry, under either branch of item 10.
...
| FI-8 | S2 — the exemplar's `rejected_envelope` test (fourth correction) | complete | sportmonks-client **111 passed** (107 intact, +4) | The frozen exemplar taught the defect four slices were rejected for. `trial_auth.py`'s `rejected_envelope` test fed one payload and asserted a substring; the literal `"data[],pagination{current_page,has_more}"` left all 107 green — measured on `main`, not supposed. That is the entry reporting the payload the parser refused, §17's top-risk observation. Now three payloads with pairwise-distinct skeletons asserted by `==`, with the distinctness itself asserted; the same literal now fails two of three. Retrofitting it exposed a second defect no single-payload test could reach: `render_skeleton` truncated below the second level, rendering `meta{pagination}` and dropping the field names — the entire observation — for any provider carrying pagination under `meta`. Standing DoD **item 11** added: two inputs, distinct expectations, equality not containment. |
