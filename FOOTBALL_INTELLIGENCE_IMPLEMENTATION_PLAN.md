...
| FI-8 | S5b — statistics: fixture-level team and player match stats | **extracted; draft — completes on trial day 5-10** | sportmonks-client **404 passed** (382 intact, +22); `python scripts/trial_stats.py --mock` exits 0 and writes both artifacts, byte-stable across repeats and matching the committed example byte-for-byte; falsifiability probe over `trial_stats.py`: **32/34 killed, seeds=34, aborts=0, in-scope survivors=0**, the two survivors being the declared-exempt `Objective` titles at `trial_stats.py:231`/`:232`; **owner-silent kills=0** — every seed was caught by `test_trial_health_stats.py`, the declared owner, not only by foreign tests; both controls passed (negative control survived, positive control killed) | Adds `scripts/trial_stats.py`, its committed mock report, and the `trial_stats.py` half of `tests/test_trial_health_stats.py` (objectives 13, 14, and the 15/16 scaffold). **This row is deliberately not `complete`.** Per the trial-day order above, S5b is slice 5 of 5 and runs on day 5-10 because stat completeness and update timing need real matches — the code is rehearsed against mock only, and objectives 13/14 stay `not_started` on the dashboard until FI-9 observes them live. Per-field presence is a **count, never a boolean**: each objective reports `field k/n` over the records that arrived, a field absent from every record degrades that objective and drops from its shape without touching the sibling, and `test_partial_field_presence_is_counted_not_rounded` pins `value=1/3` for both families so a `len(records)` substitution cannot pass. **Objectives 15 and 16 are the one sanctioned unconditional status in the slice**: no single `--mock` run can produce the second live sample they need, so both are `not_applicable (requires FI-9 live observation)` regardless of payload, and a test drives four different stat payloads through to prove the status does not move. What ships instead is the recording scaffold — `StatSample` / `diff_samples` — tested against two hand-written samples, never against two mock runs, because `MOCK_CLOCK` makes repeated mock runs overwrite the same snapshot file. **Deviation from the slice spec:** the tests live in `tests/test_trial_health_stats.py`, not the specified `tests/test_trial_stats.py` — S5a already owns that file for the sibling script and the probe OWNERS map keys on it. `trial_stats.py` joins the required CI probe gate in this change. Provenance: recovered from the superseded, unmerged `feat/fi8-s5-health-stats` ([#88](https://github.com/darutto/FPL-Platform/pull/88)), whose S5a half landed separately.
| FI-8 | S4b — lineups, formations, grid, substitutions | complete | sportmonks-client **333 passed** (299 intact, +34); falsifiability evidence comes from the **required CI probe gate**, which this change joins, rather than from a local sweep — the reasoning is recorded below because it outlives the number | Adds `scripts/trial_lineups.py`, `tests/test_trial_lineups.py`, and four formation-grid entries in `edge_cases.json` (objectives 6–10). **The risk in this slice is not a crash; it is a confident answer to a question the provider has not been asked.** `value_shape()` reports structure and never reads a value — `"1:4"` and `"4:1"` render identically, which is correct for a script whose job is to describe while §14.3 question 13 is open — and a grid whose shape differs from the documented type is **recorded and `degraded`**, not rejected and not silently passed. The same restraint governs objective 6, where a plausible answer is available and wrong to adopt: Sportmonks is widely said to mark starters with one `type_id` and bench players with another, and this script reports only the field that partitions the lineup and the distribution of its values. Naming 11 "starter" would be a semantic decision wearing a count's clothes, reported with identical confidence whether or not it were true. A field taking one value across every record degrades rather than claiming a partition it does not have. Objectives 7, 8 and 9 are statused independently and each is moved **alone** by test, because collapsing them hides §14.4's NO-GO exactly: a formation string satisfies GO criterion (b) on a technicality while the grid is absent, and that payload is driven from a checked-in fixture rather than one built in the test. Objective 10's direction is read from the field name rather than payload position — an implementation reading `player_out_id` and `player_in_id` backwards reports every row wrong and every count right — so two substitutions with the ids swapped are asserted to produce two different triples. **Probe evidence note.** The local Windows sweep was abandoned mid-run in favour of the CI gate after measuring that the same suite takes **2.4s on the Linux runner and 2–5 minutes locally**. CI swept **73 seeds across five scripts in ~4 minutes, 61/73 killed, 0 in-scope survivors**; the local sweep needed a further ~90 minutes to produce strictly less, on the platform §15 already records as the *less* likely of the two to falsify an assumption. Two local sweeps were additionally killed by session ends and left seeds on disk — the restore-skipping residue filed on [#93](https://github.com/darutto/FPL-Platform/issues/93), observed twice more here and restored by hand both times. |
| FI-8 | S6 — identity: match rate and provider-id stability | complete | sportmonks-client **336 passed** (299 intact, +37), verified **3/3 green** in a tree asserted clean before and after each run; falsifiability probe over `trial_mapping.py`: **21/23 killed, 0 in-scope survivors**, the two survivors being the declared-exempt `Objective` titles. That sweep was taken at `c14ddfe`, before a **docstring-only** correction shifted the site labels' line numbers; a docstring changes no reported value and no assertion, so it cannot change falsifiability, and the required probe gate re-sweeps this script on the PR against the final tree on a different platform rather than the evidence resting on the earlier local run | Adds `scripts/trial_mapping.py` and `tests/test_trial_mapping.py` (objectives 18 and 19). **The one slice whose output is itself a §14.1 gate**, so the instrument is pinned against a number published before it existed: `test_the_rate_reproduces_the_published_fi2_number` runs this script's arithmetic over the checked-in FI-2 corpus and requires exactly 375/461 = `0.813449` **and the same unresolved-reason breakdown** — the figure §14.1 quotes. The DoD's hand-built `8/10 → 80.0` pin is there too, but on its own it would agree with a wrong instrument just as readily as a right one. Below the gate is `unmet` and exits 1, never rounded up: the comparison is against the four-decimal rate, so 94.99996% cannot round its way in, and the unmet path is driven by the **real** 81.3449% rather than a synthetic low number. **No new matching tier** — `match_player` is asserted identical to the registry's by `is`, and `MATCH_TIERS` is pinned by value at runtime, which is the shape a shadowed table would actually take; the candidate set is assembled here only because the registry's own assembly lives in a pandas-importing module, and that mirroring is measured by the published-number test rather than asserted. **DoD 5**: the registry is read and never written, asserted by hashing the whole tree before and after a run, with the hash shown to notice a real edit so the guard is not vacuous. The mock pool is synthesized from the registry's own candidates and matches by construction — standing DoD 2 requires exit 0 on `--mock` while DoD 6 requires below-threshold to exit 1, and rehearsing against the real corpus would make failure the expected state, which is the condition under which a real regression stops being visible. `SYNTHETIC_WARNING` says in the report that the rehearsal rate is not evidence about Sportmonks. **Recorded deviation — `EXPECTED_THIRD_PARTY` grows by one entry.** Importing `football_identity_registry` trips the S2 allowlist pinning the live-call guard's completeness. That pin's own error message sanctions two ways to grow: extend the conftest guard, or record why the library cannot reach the network. The second applies, recorded as a **measurement** rather than a comment — the registry does import `pandas` and `yaml`, in three modules not on this import path, so a file scan would report a dependency that never executes. The test measures what a fresh interpreter actually loads, as a **difference** against the harness alone (since `requests` brings `http`, `socket` and `ssl` with it and those are the routes the guard already covers), with an instrument test ahead of it and a non-vacuity assertion behind it. |
