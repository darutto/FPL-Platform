...
| FI-8 | S5 — health and statistics | implemented; under review | sportmonks-client **144 passed** (107 pre-existing intact, +37 new) | `trial_injuries.py` reports injuries, suspensions, and coach/manager records as three separately-statused objectives (`Objective(11, "Injuries")`, `Objective(11, "Suspensions")`, `Objective(12, ...)`); every injury record's freshness is read from its own `updated_at` field, and a record with no freshness value (absent, null, or empty) is reported `degraded`, never defaulted to fresh. `trial_stats.py` reports fixture-level team statistics and player match statistics as two objectives, each with per-field presence counts (`name=count/total`) derived from the records received; a field absent from every record degrades that objective and drops that field from its shape without touching the sibling objective. Objectives 15 and 16 (update timing, post-match corrections) are shipped as a recording scaffold only — `StatSample`/`diff_samples` in `trial_stats.py` — and are marked `not_applicable (requires FI-9 live observation)` unconditionally in mock mode; the diff logic is tested against two hand-written `StatSample` instances, never against two mock runs of the script (`MOCK_CLOCK` makes those overwrite the same file). **Note:** implemented directly per explicit task scope; S3 (`trial_entities.py`/`trial_fixtures.py`) and S4 (`trial_squads.py`/`trial_lineups.py`) remain unimplemented on `main` at this slice's branch point, so the "previous slice" baseline for standing DoD item 1 is S2's 107, not a hypothetical post-S4 count. |
