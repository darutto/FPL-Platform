...
| 2026-08-19 | [Agentic-loop class-2 grader fixes + composition wiring](2026-08-19-agentic-loop-class2-grader-fixes.md) | Fixture grounding lookup bug (int vs string keys); test masking bug; composition check never called; rendering collision (0/3 with catastrophic rate); unknown status renders as measured result | fixtures, orchestrator, instruments, tooling | fixed |
| 2026-08-28 | [Capitanía en prod — 3 respuestas distintas + chip que contesta otra pregunta](2026-08-28-captain-answer-nondeterminism-prod.md) | **(a)** misma pregunta → 3+ respuestas (una es un volcado de jornada); primera instancia **en producción** del atractor `get_gameweek_context` medido offline en i38; se compone con el gap de `_TOOL_TO_INTENT` (tool sin mapeo ⇒ sin tarjeta ⇒ texto crudo); ninguna compuerta lo ve (`outcome ok`, `substantive_answer`, catastrophic `0/3`); `/capitan` pelado sin `suggestions`. **(b)** `get_chip_advice` no acepta jornada ni jugador (schema de 1 propiedad) ⇒ «TC a Haaland en la fecha 3» se contesta sobre la GW2 y sobre Cherki, y el consejo sale **al revés** (GW3 vs COV FDR 2 es el mejor fixture, verificado en vivo); la tarjeta omite de quién es su 79.0 y **contradice al texto**; disclaimer de disponibilidad de chip obsoleto desde 8e1 | orchestrator, contracts, ui, gw-resolution, chips, scoring | new |
| 2026-09-08 | [Newcastle-Leeds J4 en prod](2026-09-08-newcastle-leeds-prod-session.md) | **el boton de calendario dispara `fixtureCellQuestion()` y falla en su primer intento** con volcado de GW4 (`get_fixtures_for_gw` como atractor nuevo, distinto del `get_gameweek_context` de i38); misma pregunta x3 -> 2 tools, 2 respuestas; no-respuesta de 79 chars por 52.817 tokens con `outcome ok`; **`retry_attempted` cableado a False y desmentido por `retry_input` del mismo registro**; **los turnos de `/session/{id}/ask` llegan como `anonymous`/`free`** porque su route.ts no reenvia `x-user-id` (bucket de cuota compartido + tier premium perdido); audit de sesion con `tokens={}` y `tool_calls=[]` constantes -> coste cero y routing ciego por construccion; **`MarkdownLite` no soporta `#` headers ni listas numeradas -> `###`/`1.` literales en la burbuja** | orchestrator, fixtures, contracts, instruments, ui, data-quality | new |
