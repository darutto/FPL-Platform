> **SUPERSEDED — kept on purpose.** Correct instrument (preflight, stale
> exclusion), but `invented_metric_relay` gated on a stricter criterion —
> requiring the relay to have HAPPENED — that was reverted: "refuse rather than
> reinterpret" is a product policy nobody had decided, and a gate encodes decided
> policy rather than inventing it (now card i48). Its 9/15 survives as the
> companion figure. See the REFERENCE row beside it.
