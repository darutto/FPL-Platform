...
def main(argv=None) -> int:
    ...
    replay = sub.add_parser("replay"); replay.add_argument("--manifest", type=Path, required=True); replay.add_argument("--destination", type=Path, required=True); replay.add_argument("--source", type=Path)
    ...
    try:
        ...
        else: report = replay_manifest(args.manifest, args.destination, args.source)
