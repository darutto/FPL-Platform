...
RANK_CAPTAIN_CANDIDATES_SCHEMA = ToolSchema(
    ...
    description=(
        ...
        "When a team is connected, the derived result also retains every "
        "eligible owned midfielder/forward and marks entries with owned=true, "
        "so present owned candidates separately from the true global top. "
        "Only MID/FWD are captain candidates; squad_excluded accounts for every "
        "other owned player with an explicit exclusion reason. "
        "If squad_source is not_connected or unavailable, say that only the "
        "global ranking is shown. "
