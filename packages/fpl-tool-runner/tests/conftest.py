"""
Shared fixtures for fpl-tool-runner tests.

Mirrors the fpl-tool-contract fixture shape so the tool-runner tests
exercise the same player data without duplicating logic.

Raw elements use FPL API key \"team\" (not \"team_id\") so that
fpl_api_client.get_players(bootstrap) normalises the key correctly.
"""
import copy
import pytest
...
_RAW_ELEMENTS = [
    ...
    # Duplicate web_name pair — triggers ambiguous
