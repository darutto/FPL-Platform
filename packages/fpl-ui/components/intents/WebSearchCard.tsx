/**
 * WebSearchCard — structured rendering for an unverified web-search answer
 * (search_web tool, premium path).
 *
 * Rendered beneath final_text when response.web_search is present (see
 * lib/intent-renderer.ts selectIntentView). Ported from WcWebSearchCard.tsx
 * (World Cup chat) — same cyan accent (the system's "web/search" color), a
 * globe provenance strip, the rendered `summary`, the cited results, and a
 * clickable sources footer.
 *
 * UNLIKE the deterministic cards, this content is AI synthesis over live web
 * sources — NOT grounded FPL data. The "sin verificar" banner + the cyan
 * "Búsqueda web + IA" origin badge (MessageList) make that explicit so it
 * never reads as a deterministic answer.
 */
  ...
  return (
    <div className={`mt-3 text-sm ${CARD_BASE} ${CARD_ACCENT.cyan.border}`}>
      ...
      <div className="px-4 py-3 space-y-2">
        ...
        {/* Rendered synthesis (== final_text). Minimal markdown (bold + bullets). */}
        {summary && (
